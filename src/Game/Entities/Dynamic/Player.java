...
public class Player extends BaseDynamicEntity {
	...
	private void ringCustomer(Client client) {
		...
		if(matched){
			...
			if (client.inspector) {
				gReview = true;
				bReview = false;
				handler.getWorld().clients.forEach(customer -> customer.patience+=customer.patience*0.12);
			}
			if(wellDone) {
	...
	public void render(Graphics g) {
		...
		g.drawString("Earnings: " + formatt.format(money), handler.getWidth()/2 -90, 52);
		g.drawString("Served: " + served +"  Left: "+gone, handler.getWidth()/2 -90, 76);
	}
